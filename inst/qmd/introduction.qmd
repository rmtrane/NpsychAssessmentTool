...
if (!file.exists(here::here("inst/qmd/carousel.yml")) & !file.exists("carousel.yml")) {
  tmp <- getwd()
...
Multiple standardization methods are implemented in the R-package [`NpsychBatteryNorms`](https://rmtrane.github.io/NpsychBatteryNorms), though not all methods are available for all variables. 
...
all_methods <- c("norms", "regression", "T-score")
...
for_tab <- map(
  ...
  \(mth) {
    tab <- tibble(
      version = {
        if (mth == "norms") {
          out <- names(NpsychBatteryNorms::normative_summaries)
        }

        if (mth == "regression") {
          out <- names(NpsychBatteryNorms::reg_coefs)
        }
      ...
      mutate(
        short_descriptor = purrr::map_chr(
          variables,
          \(x) {
            out <- purrr::pluck(NpsychBatteryNorms::rdd, x, "short_descriptor")
    ...
    if (mth != "norms") {
      tab <- left_join(
        ...
        NpsychBatteryNorms::default_methods |>
          ...
          mutate(
            default = T,
            version = if_else(method == "T-score", "T", version) # replace_na(version, "T")
          )
      )
    } else {
      tab$default <- NA
